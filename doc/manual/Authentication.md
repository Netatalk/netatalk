...
- **SRP** uses the *afppasswd.srp* directory, with one mode-0600 salt and
  verifier file per numeric uid. Initialize root-owned disabled placeholders
  with **afppasswd -c**, then set each password as root with **afppasswd -a**
  *username*. Setting a real verifier transfers ownership to the user and
  enables SRP login; enrolled users can then update their own verifier without
  an elevated executable. See **afppasswd**(1) for password-policy limits and
  how to disable SRP access.
