...
cups_get_printer_status(struct printer *pr)
{
    ...
    if (!printer_uri_supported
            || !strcmp(printer_is_temporary,
                       "true")) { /* DNS-SD discovered IPP printer: Get status directly from printer */
        flags = CUPS_DEST_FLAGS_DEVICE;
    } else {
        flags = CUPS_DEST_FLAGS_NONE;
    }
...
cups_autoadd_printers(struct printer	*defprinter, struct printer *printers)
{
    ...
    num_dests = cupsGetDests2(CUPS_HTTP_DEFAULT,
                              &dests);	/* get the available destination from CUPS */
                       ...
                       int replace)
{
    ...
    while (listptr != NULL) {
        if (strcasecmp(pr->p_name, listptr->p_name) == 0) {
            if (pr->p_flags &
                    P_CUPS_AUTOADDED) {    /* Check if printer has been autoadded */
