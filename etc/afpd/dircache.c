...
/*
 * Configuration variable to control whether files are allowed in the dircache.
 * When false (default), files are rejected.
 */
bool dircache_files_enabled = false;  // Default: false (reject files)
...
static unsigned long queue_count_max =
    0;  /* Peak entries reached this session */
...
struct dir *dircache_search_by_did(const struct vol *vol, cnid_t cnid)
{
     ...
     * Assert for other invalid CNIDs (1-16) which would indicate a bug. */
...
int dircache_remove_children(const struct vol *vol, struct dir *dir)
{
    ...
    for (int i = 0; i < remove_count; i++) {
        ...
        if (dir_remove(vol, entry) != 0) {
            recovery_status = -1;  /* Propagate failure */
        }
    }
