...
/* Per-entry limit (bytes), 0 = disabled */
...
/*!
 * @brief Shutdown the rfork cache — free remaining RFork LRU nodes and sentinel
 *
 * Called from the child shutdown path after log_dircache_stat().
 * Frees the qnodes (rfork data buffers are reclaimed by exit()) and the LRU sentinel.
 */
void dircache_rfork_shutdown(void)
{
    if (rfork_lru != NULL) {
        /* Free remaining LRU nodes. struct dir entries still exist but
         * the process is about to exit — just unlink and free each qnode. */
...
void dircache_dump(void)
{
    ...
    snprintf(tmpnam, sizeof(tmpnam) - 1, "%s/dircache.%u", tmpdir(), AFPobj->pid);
