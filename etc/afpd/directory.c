 ...
 * 2. Search dircache:
 *    a. If found and valid, return cached entry
 *    b. If strict mode, validate inode before use
...
#define CNID_PATH_OVERHEAD 12  /* CNID protocol header size for path resolution */
    ...
    if ((ret = dircache_search_by_did(vol, did)) != NULL) { /* 2a */
        /* Cache HIT */
        if (!dircache_files_enabled && (ret->d_flags & DIRF_ISFILE)) {
    ...
    if (ostat(cfrombstr(fullpath), &st, vol_syml_opt(vol)) != 0) { /* 5a */
        ...
        if (!dircache_files_enabled && !S_ISDIR(st.st_mode)) {
...
int dir_remove(const struct vol *vol, struct dir *dir)
{
    ...
    if (removing_curdir && !curdir) {
        ...
        if (recovered) {
            ...
            return 0;  /* Success - curdir recovered */
        }
        ...
        return -1;  /* FAILURE - caller must handle */
    }

    return 0;  /* Success - curdir unchanged or not affected */
}
...
struct path *cname(struct vol *vol, struct dir *dir, char **cpath)
{
    ...
    int         toUTF8 = 0;
    int         original_len =
        0;  /* Save original len before while loop modifies it */
    ...
    *cpath += len + size;
    path[0] = 0;
    ret.m_name = path;
    original_len = len;  /* Save before while loop modifies it */
    ...
    if (ret.m_name[0] == 0) {
        ...
        ret.d_dir = curdir;  /* Use curdir (updated by movecwd, handles self-healing) */
    } else if (original_len == 0) {
        /* Special case: original_len was 0 from start (statting directory itself via empty path).
         * ret.m_name[0] is not 0 because line 1389 never executed (while loop skipped).
        ...
        ret.m_name[0] = 0;  /* Mark as directory */
        ret.u_name = mtoupath(vol, ret.m_name, 0, 1);  /* "." */
        ret.d_dir = curdir;  /* Use curdir (updated by movecwd, includes healing) */
        LOG(log_info, logtype_afpd,
        ...
        if ((set_maccess || set_upriv) && dir) {
            ...
            if (ostat(upath, &st, vol_syml_opt(vol)) == 0) {
                ...
                if (dir == curdir) {
                    ...
                    dir->d_rights_cache = 0xffffffff;
                    /* Use global AFPobj (always set during AFP session, see afp_dsi.c:498) */
