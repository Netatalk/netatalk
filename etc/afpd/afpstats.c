...
static gpointer afpstats_thread(gpointer _data _U_)
{
    ...
    if (!(bus = g_bus_get_sync(G_BUS_TYPE_SYSTEM, NULL, &error))) {
        LOG(log_error, logtype_afpd,
            "afpstats_thread: Couldn't connect to system bus: %s", error->message);
        g_error_free(error);
        main_loop = NULL;
    ...
    if (!introspection_data) {
        LOG(log_error, logtype_afpd,
            "afpstats_thread: Failed to parse introspection data: %s",
    ...
    if (registration_id == 0) {
        LOG(log_error, logtype_afpd, "afpstats_thread: Failed to register object: %s",
            error ? error->message : "Unknown error");
        g_clear_error(&error);
        g_object_unref(obj);
        g_dbus_node_info_unref(introspection_data);
...
int afpstats_init(server_child_t *childs_in)
{
    ...
    if (thread == NULL) {
        LOG(log_error, logtype_afpd, "afpstats_init: failed to create thread: %s",
            error->message);
        fprintf(stderr, "afpstats_init: Could not start AFPStats D-Bus thread: %s\n",
                error->message);
        g_error_free(error);
        return -1;
    }
