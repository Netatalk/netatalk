...
struct scrit {
    ...
    uint16_t fbitmap,
             dbitmap; /* file & directory bitmap - which values should we return ? */
...
static int crit_check(struct vol *vol, struct path *path)
{
    ...
    if (S_ISDIR(path->st.st_mode)) {
        ...
        */
        if (!(path->m_name = utompath(vol, path->u_name, 0,
    ...
    if (c1.rbitmap & (1U << DIRPBIT_LNAME)) {
        ...
        } else if (strcasecmp_w((ucs2_t *) convbuf, (ucs2_t *) c1.lname) != 0) {
            goto crit_check_ret;
        }
    }
    ...
    if (c1.rbitmap & (1U << FILPBIT_PDINFO)) {
        ...
        } else if (strcasecmp_w((ucs2_t *)convbuf, (ucs2_t *)c1.utf8name) != 0) {
            goto crit_check_ret;
        }
    }
...
{
    static uint32_t
    cur_pos;    /* Saved position index (ID) - used to remember "position" across FPCatSearch calls */
    static DIR
    *dirpos; 		 /* UNIX structure describing currently opened directory. */
    ...
    while ((cidx = reducestack()) != -1) {
        ...
        LOG(log_debug, logtype_afpd, "catsearch: current struct dir: \"%s\"",
        ...
        if (dirpos == NULL) {
            const char *temp = cfrombstr(currentdir->d_fullpath);
            dirpos = opendir(temp);
        }
                         ...
                         char *rbuf, size_t *rbuflen, int ext)
{
    ...
    *rbuflen = 0;
    ...
    if (c1.rbitmap & (1U << FILPBIT_PDINFO)) {
        ...
        len = convert_charset(CH_UTF8_MAC, CH_UCS2, CH_UTF8, c1.utf8name, namelen,
                              c1.utf8name, 512, &flags);
