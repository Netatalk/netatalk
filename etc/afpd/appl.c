...
static int applopen(struct vol *vol, unsigned char creator[ 4 ], int flags, int mode)
{
    char	*dtf, *adt, *adts;
 ...
 */
static int copyapplfile(int sfd, int dfd, char *mpath, unsigned short mplen)
{
    int		cc;
    char	*p;
    uint16_t	len;
    ...
    while (( cc = read( sfd, buf, sizeof(appltag) + sizeof( unsigned short ))) > 0 ) {
...
int afp_getappl(AFPObj *obj, char *ibuf, size_t ibuflen _U_, char *rbuf, size_t *rbuflen)
{
    ...
    memset(appltag, 0, sizeof(unsigned char) * 4);
    ibuf += 2;
