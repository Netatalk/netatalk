...
int setdeskmode(const struct vol *vol, const mode_t mode)
{
    ...
    if (ochmod(bdata(dtpath),
        ...
        LOG(log_error, logtype_afpd, "setdeskmode: chmod %s: %s", bdata(dtpath),
            strerror(errno));
    }
                ...
                size_t *rbuflen)
{
    ...
    *rbuflen = 0;
    ...
    if (lseek(si.sdt_fd, (off_t) 0L, SEEK_SET) < 0) {
        close(si.sdt_fd);
        si.sdt_fd = -1;
        LOG(log_error, logtype_afpd, "afp_addicon(%s): lseek: %s", icon_dtfile(vol,
...
addicon_err:
    ...
    case AFPPROTO_ASP:
        ...
        if (writev(si.sdt_fd, iov, iovcnt) < 0) {
            LOG(log_error, logtype_afpd, "afp_addicon(%s): writev: %s", icon_dtfile(vol,
                    fcreator), strerror(errno));
            return AFPERR_PARAM;
        }
    ...
    case AFPPROTO_DSI: {
        DSI *dsi = obj->dsi;
        char *wbuf = NULL;
        size_t wlen = dsi_writeinit(dsi, &wbuf);
        ...
        if (wlen > 0 && write(si.sdt_fd, wbuf, wlen) < 0) {
            LOG(log_error,
                logtype_afpd,
                "afp_addicon(%s): write: %s",
                icon_dtfile(vol, fcreator),
        ...
        while ((wlen = dsi_write(dsi, rbuf, buflen))) {
            if (write(si.sdt_fd, rbuf, wlen) < 0) {
                LOG(log_error,
                    logtype_afpd,
                    "afp_addicon(%s): write: %s",
                    icon_dtfile(vol, fcreator),
    ...
    *rbuflen = 0;
    ...
    for (;;) {
        ...
        if (lseek(si.sdt_fd, (off_t) bsize, SEEK_CUR) < 0) {
            LOG(log_error, logtype_afpd, "afp_iconinfo(%s): lseek: %s", icon_dtfile(vol,
                    fcreator), strerror(errno));
            return AFPERR_PARAM;
        }
                ...
                size_t *rbuflen)
{
    ...
    *rbuflen = 0;
    ...
    if (lseek(si.sdt_fd, (off_t) 0L, SEEK_SET) < 0) {
        close(si.sdt_fd);
        si.sdt_fd = -1;
        LOG(log_error, logtype_afpd, "afp_geticon(%s): lseek: %s", icon_dtfile(vol,
                fcreator), strerror(errno));
        return AFPERR_PARAM;
    }
    ...
    if (rc < 0) {
        LOG(log_error, logtype_afpd, "afp_geticon(%s): read: %s", icon_dtfile(vol,
                fcreator), strerror(errno));
        return AFPERR_PARAM;
    }
...
#define min(a,b)	((a)<(b)?(a):(b))
    ...
    if ((obj->proto == AFPPROTO_DSI) && buflen < rc) {
        ...
        *rbuflen = buflen;
        ...
        return AFP_OK;
geticon_exit:
        LOG(log_error, logtype_afpd, "afp_geticon(%s): %s", icon_dtfile(vol, fcreator),
            strerror(errno));
        dsi_readdone(dsi);
...
char *utompath(const struct vol *vol, char *upath, cnid_t id, int utf8)
{
    ...
    LOG(log_debug9, logtype_afpd, "utompath: '%s':'%s':'%2.2X'", upath, m,
        ntohl(id));
    return m;
utompath_error:
    u = "???";
                         ...
                         size_t *rbuflen)
{
    ...
    /* OSX only use 128, greater kill Adobe CS2 */
    clen = min(ad_getentrylen(adp, ADEID_COMMENT), 128);
    *rbuf++ = clen;
...
static int ad_rmvcomment(const AFPObj *obj, struct vol *vol, struct path *path)
{
    ...
    if (ad_open(adp, upath, ADFLAGS_HF | ADFLAGS_RDWR | (isadir ? ADFLAGS_DIR : 0))
            < 0) {
        switch (errno) {
        case ENOENT :
            return AFPERR_NOITEM;
