...
static int dbif_stamp(DBD *dbd, void *buffer, int size)
{
    ...
    if ((chdir(dbd->db_envhome)) != 0) {
        LOG(log_error, logtype_cnid, "error chdiring to db_env '%s': %s",
            dbd->db_envhome, strerror(errno));
        EC_FAIL;
    }

    if (stat(dbd->db_table[DBIF_CNID].name, &st) < 0) {
        LOG(log_error, logtype_cnid, "error stating database %s: %s",
...
static int dbif_openlog(DBD *dbd)
{
    ...
    if ((dbd->db_errlog = fopen(DB_ERRLOGFILE, "a")) == NULL) {
        LOG(log_warning, logtype_cnid, "error creating/opening DB errlogfile: %s",
            strerror(errno));
    }
...
static int dbif_logautorem(DBD *dbd)
{
    ...
    if (logfiles != NULL) {
        for (file = logfiles; *file != NULL; file++) {
            if (unlink(*file) < 0) {
                LOG(log_warning, logtype_cnid, "Error removing stale logfile %s: %s", *file,
                    strerror(errno));
            }
        }
...
int dbif_env_open(DBD *dbd, struct db_param *dbp, uint32_t dbenv_oflags)
{
    ...
    if (dbenv_oflags & DB_RECOVER) {
        ...
        if ((ret = dbd->db_env->open(dbd->db_env, dbd->db_envhome,
    ...
    if ((ret = dbd->db_env->set_cachesize(dbd->db_env, 0, 1024 * dbp->cachesize,
                                          0))) {
...
int dbif_open(DBD *dbd, struct db_param *dbp, int reindex)
{
    ...
    if (dbd->db_envhome) {
        ...
        if ((stat(dbd->db_filename, &st)) == 0) {
            ...
            if ((ret = db_create(&upgrade_db, dbd->db_env, 0))) {
                LOG(log_error, logtype_cnid, "error creating handle for database: %s",
                    db_strerror(ret));
                return -1;
            }
    ...
    if ((dbd->db_envhome) && ((ret = dbif_upgrade(dbd)) != 0)) {
        LOG(log_error, logtype_cnid, "Error upgrading CNID database to version %d",
            CNID_VERSION);
        return -1;
    }
...
static int dbif_closedb(DBD *dbd)
{
    ...
    for (i = DBIF_DB_CNT - 1; i >= 0; i--) {
        if (dbd->db_table[i].db != NULL
                && (ret = dbd->db_table[i].db->close(dbd->db_table[i].db, 0))) {
            LOG(log_error, logtype_cnid, "error closing database %s: %s",
                dbd->db_table[i].name, db_strerror(ret));
            err++;
        }
    }
...
int dbif_close(DBD *dbd)
{
    ...
    if (dbd->db_env != NULL && (ret = dbd->db_env->close(dbd->db_env, 0))) {
        LOG(log_error, logtype_cnid, "error closing DB environment: %s",
            db_strerror(ret));
        err++;
    }
...
int dbif_env_remove(const char *path)
{
    ...
    if ((ret = dbd->db_env->open(dbd->db_env,
                                 dbd->db_envhome,
                                 DB_CREATE | DB_INIT_LOG | DB_INIT_MPOOL | DB_INIT_LOCK | DB_INIT_TXN |
    ...
    if ((ret = dbd->db_env->log_archive(dbd->db_env, NULL, DB_ARCH_REMOVE))) {
        LOG(log_error, logtype_cnid, "error removing transaction logfiles: %s",
            db_strerror(ret));
        return -1;
    }
    ...
    if ((ret = db_env_create(&dbd->db_env, 0))) {
        LOG(log_error, logtype_cnid, "error acquiring db_end handle: %s",
            db_strerror(ret));
        dbd->db_env = NULL;
        return -1;
    }
    ...
    if ((ret = dbd->db_env->remove(dbd->db_env, dbd->db_envhome, 0))) {
        LOG(log_error, logtype_cnid, "error removing BerkeleyDB environment: %s",
            db_strerror(ret));
        return -1;
    }
...
int dbif_put(DBD *dbd, const int dbi, DBT *key, DBT *val, uint32_t flags)
{
    ...
    if (dbif_txn_begin(dbd) < 0) {
        LOG(log_error, logtype_cnid, "error setting key/value in %s",
...
int dbif_del(DBD *dbd, const int dbi, DBT *key, uint32_t flags)
{
    ...
    if (dbif_txn_begin(dbd) < 0) {
        LOG(log_error, logtype_cnid, "error deleting key/value from %s",
            dbd->db_table[dbi].name);
        return -1;
    }
 ...
 * @param[in]  key     initialised with the search name (key->data, key->size)
 ...
 *
 * @returns -1 on BDB engine error, else the number of matches written
                ...
                uint32_t offset, bool *more)
{
    ...
    uint32_t skipped = 0;
    DBC *cursorp = NULL;
    /* `data` is the secondary-DB record payload, required by BDB's pget()
     * out-parameter API but unused here; we only need the primary-key
     * (CNID) bytes from `pkey`. */
    DBT pkey, data;
    char *cnids = resbuf;
    cnid_t cnid;
    char *namebkp = key->data;
    /* DBT::size is u_int32_t */
    size_t namelenbkp = (size_t)key->size;

    /* API contract: caller MUST supply a backing bool. cnid_dbd is a
     * single-threaded long-lived daemon — a NULL deref here would SIGSEGV
     * the daemon and interrupt CNID service for every afpd worker
     * currently attached to this volume (one cnid_dbd per volume; users
     * of OTHER volumes are unaffected because each volume has its own
     * cnid_dbd child). cnid_metad would respawn the daemon on the next
     * client request, but in-flight queries from afpd workers attached
     * to this volume would fail. Fail closed instead. */
    if (more == NULL) {
        LOG(log_error, logtype_cnid,
            "dbif_search: NULL `more` out-parameter (caller bug)");
        return -1;
    }
    ...
    *more = false;
    ...
    ret = dbd->db_table[DBIF_IDX_NAME].db->cursor(
              dbd->db_table[DBIF_IDX_NAME].db, NULL, &cursorp, 0);

    if (ret != 0) {
        LOG(log_error, logtype_cnid,
            "dbif_search: cursor open failed: %s", db_strerror(ret));
        ret = -1;
        goto exit;
    }
    ...
    while (true) {
        ...
        if (ret != 0) {
            LOG(log_error, logtype_cnid,
                "dbif_search: pget failed: %s", db_strerror(ret));
            ret = -1;
            goto exit;
        }
...
int dbif_txn_begin(DBD *dbd)
{
    ...
    if (ret) {
        LOG(log_error, logtype_cnid, "error starting transaction: %s",
            db_strerror(ret));
        return -1;
    } else {
        return 0;
...
int dbif_txn_commit(DBD *dbd)
{
    ...
    if (ret) {
        LOG(log_error, logtype_cnid, "error committing transaction: %s",
            db_strerror(ret));
        return -1;
    } else {
        return 1;
...
int dbif_txn_abort(DBD *dbd)
{
    ...
    if (ret) {
        LOG(log_error, logtype_cnid, "error aborting transaction: %s",
            db_strerror(ret));
        return -1;
    } else {
        return 0;
                        ...
                        uint32_t flags)
{
    ...
    if (ret) {
        LOG(log_error, logtype_cnid, "error checkpointing transaction susystem: %s",
            db_strerror(ret));
        return -1;
    } else {
        return 0;
...
int dbif_count(DBD *dbd, const int dbi, uint32_t *count)
{
    ...
    if (ret) {
        LOG(log_error, logtype_cnid, "error getting stat infotmation on database: %s",
            db_strerror(ret));
        return -1;
    }
