...
struct adouble {
    uint32_t
    ad_magic;         /* Official adouble magic                   */
    uint32_t
    ad_version;       /* Official adouble version number          */
    char                ad_filler[16];
    struct ad_entry     ad_eid[ADEID_MAX];

    struct ad_fd
        ad_data_fork;     /* the data fork                            */

    struct ad_fd
        ad_resource_fork; /* adouble:v2 -> the adouble file           *
                                           * adouble:ea -> the EA fd                  */

    struct ad_fd
        *ad_rfp;          /* adouble:v2 -> ad_resource_fork           *
                                           * adouble:ea -> ad_resource_fork           */

    struct ad_fd
        *ad_mdp;          /* adouble:v2 -> ad_resource_fork           *
                                           * adouble:ea -> ad_data_fork               */

    int
    ad_vers;          /* Our adouble version info (AD_VERSION*)   */
    int
    ad_adflags;       /* ad_open flags adflags like ADFLAGS_DIR   */
    uint32_t            ad_inited;
    int                 ad_options;
    int
    ad_refcount;       /* multiple forks may open one adouble     */
    int                 ad_data_refcount;
    int                 ad_meta_refcount;
    int                 ad_reso_refcount;
    off_t
    ad_rlen;           /* resource fork len with AFP 3.0         *
                                            * the header parameter size is too small. */
    char
    *ad_name;          /* mac name (maccharset or UTF8-MAC)       */
    struct adouble_fops *ad_ops;
    uint16_t
    ad_open_forks;     /* open forks (by others)                  */
    size_t
    valid_data_len;	   /* Bytes read into ad_data                 */
