...
typedef struct afp_child {
    pid_t
    afpch_pid;         /* afpd worker process pid (from the worker afpd process )*/
    uid_t
    afpch_uid;         /* user id of connected client (from the worker afpd process) */
    int             afpch_valid;       /* 1 if we have a clientid */
    int
    afpch_killed;      /* 1 if we already tried to kill the client */
    ...
    int
    afpch_ipc_fd;      /* socket for IPC bw afpd parent and childs */
    int16_t
    afpch_state;       /* state of AFP session (eg active, sleeping, disconnected) */
...
typedef struct {
    ...
    int
    servch_count;                   /* Current count of active AFP sessions */
    int
    servch_nsessions;               /* Number of allowed AFP sessions */
    afp_child_t
    *servch_table[CHILD_HASHSIZE];   /* Hashtable with data of AFP sesssions */
} server_child_t;
