/*
 * $Id: cnid_db3_add.c,v 1.5 2009-10-29 13:38:16 didg Exp $
 *
...
/* #ifdef DEBUG */
#if 0
        LOG(log_debug9, logtype_default, "cnid_add: Found rootinfo for did %u, name %s as %u", ntohl(did), name, ntohl(hint));
#endif
        break;
    case DB_NOTFOUND:
...
/* #ifdef DEBUG */
#if 0
        LOG(log_debug9, logtype_default, "cnid_add: Using CNID_START for did %u, name %s", ntohl(did), name);
#endif
        break;
    default:
                ...
                cnid_t hint)
{
    ...
    if (ntohl(hint) >= CNID_START) {
        ...
        case 0:
#ifdef DEBUG
            LOG(log_debug9, logtype_default, "cnid_add: Used hint for did %u, name %s as %u", ntohl(did), name, ntohl(hint));
#endif
            return hint;
        }
    }
