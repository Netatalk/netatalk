...
/*
 * $Id: cnid_db3_lookup.c,v 1.5 2009-10-29 13:38:16 didg Exp $
 */
...
#ifdef DEBUG
        LOG(log_debug9, logtype_default, "cnid_lookup: Running database checkpoint");
#endif
...
#ifdef DEBUG
        LOG(log_debug9, logtype_default, "cnid_lookup: Looked up did %u, name %s, as %u", ntohl(did), name, ntohl(id));
#endif
        return id;
    }
...
#ifdef DEBUG
    LOG(log_debug9, logtype_default, "cnid_lookup: Looked up did %u, name %s, as %u (needed update)", ntohl(did), name,
        ntohl(id));
#endif
    return id;
}
