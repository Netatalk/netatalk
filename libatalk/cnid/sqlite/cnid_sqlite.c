        ...
        if (id == CNID_INVALID) {
            ...
            if ((uint32_t) lastid > 0xffffffff) {
                ...
                if (cnid_sqlite_execute(db->cnid_sqlite_con,
                                        ...
                                        db->cnid_sqlite_voluuid_str) < 0) {
                    cnid_sqlite_execute(db->cnid_sqlite_con, "ROLLBACK");
                    LOG(log_error, logtype_cnid, "cnid_sqlite_add: sqlite query error: %s",
                        sqlite3_errmsg(db->cnid_sqlite_con));
                    EC_FAIL;
                }
...
struct _cnid_db *cnid_sqlite_open(struct cnid_open_args *args)
{
    ...
    if (bdata(dbpath) && chmod(bdata(dbpath), 0666) != 0) {
