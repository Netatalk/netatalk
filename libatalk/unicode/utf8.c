...
static size_t utf8_pull(void *cd _U_, char **inbuf, size_t *inbytesleft,
    ...
    while (*inbytesleft >= 1 && *outbytesleft >= 2) {
        ...
        if ((c[0] & 0x80) == 0) {
            ...
            if (c[0] > 0xf4) {
                goto ilseq;    /* happens for surrogate pairs only */
            }

            if (!((c[0] == 0xf0                 && 0x90 <= c[1] && c[1] <= 0xbf)
                    || /* F0    90-BF */
                    (0xf1 <= c[0] && c[0] <= 0xf3 && (c[1] & 0xc0) == 0x80)
                    ||        /* F1-F3 80-BF */
                    (c[0] == 0xf4
                     && (c[1] & 0xc0) == 0x80))) {       /* F4    80-8F */
                goto ilseq;
            }
