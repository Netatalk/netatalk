...
ucs2_t *strcasechr_sp(const ucs2_t *s, uint32_t c_sp) {
    ...
    while (s[1] != 0) {
        if (tolower_sp(c_sp) == tolower_sp((uint32_t) * s << 16 | (uint32_t)s[1])) {
            return (ucs2_t *)s;
        }
    ...
    while (i < inplen) {
        ...
        /* Binary Search for Surrogate Pair */
            ...
            if (i + 2 < inplen) {
                ...
                do {
                    if (!(result_sp = do_decomposition_sp(base_sp))) {
                        break;
                    }
...
size_t utf8_charlen(char* utf8) {
    ...
    if (*p < 0x80) {
        ...
        return 2;
    } else if (*p == 0xe0 && *(p + 1) > 0x9f && *(p + 1) < 0xc0 && *(p + 2) > 0x7f
               && *(p + 2) < 0xc0) {
        return 3;
    } else if (*p > 0xe0  && *p < 0xf0 && *(p + 1) > 0x7f && *(p + 1) < 0xc0
               && *(p + 2) > 0x7f && *(p + 2) < 0xc0) {
        return 3;
    } else if (*p == 0xf0 && *(p + 1) > 0x8f && *(p + 1) < 0xc0 && *(p + 2) > 0x7f
               && *(p + 2) < 0xc0 && *(p + 3) > 0x7f && *(p + 3) < 0xc0) {
        return 4;
    } else if (*p > 0xf0 && *p < 0xf4 && *(p + 1) > 0x7f && *(p + 1) < 0xc0
               && *(p + 2) > 0x7f && *(p + 2) < 0xc0 && *(p + 3) > 0x7f && *(p + 3) < 0xc0) {
        return 4;
    } else if (*p == 0xf4 && *(p + 1) > 0x7f && *(p + 1) < 0x90 && *(p + 2) > 0x7f
               && *(p + 2) < 0xc0 && *(p + 3) > 0x7f && *(p + 3) < 0xc0) {
        return 4;
    } else {
        return (size_t) -1;
