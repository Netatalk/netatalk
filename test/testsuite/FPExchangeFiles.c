...
STATIC void test531()
{
    ...
    if (!Quiet) {
        fprintf(stdout,
                "FPExchangeFiles:test531: File exchange with cache coherency\n");
    }
    ...
    if (FPRead(Conn, fork, 0, strlen(data1), read_buffer)) {
        test_failed();
        goto fin;
    }

    read_buffer[strlen(data1)] = '\0';
    ...
    read_fork(Conn, vol, DIRDID_ROOT, file1, strlen(data2));
    ...
    read_fork(Conn, vol, DIRDID_ROOT, file2, strlen(data1));
