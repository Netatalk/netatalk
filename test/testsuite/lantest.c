...
static int cache_dirs = 10;                       /* directories for cache tests */
static int cache_files_per_dir = 10;              /* files per directory */
static int cache_lookup_iterations = 100;         /* cache hit test iterations */
static int mixed_cache_files = 200;               /* mixed operations file count */
static int deep_dir_levels = 20;                  /* deep traversal directory levels */
static int deep_test_files = 50;                  /* files in deepest directory */
static int deep_traversals = 50;                  /* deep path traversal iterations */
static int validation_files = 100;                /* validation test file count */
static int validation_iterations = 100;           /* validation test iterations */
...
static void addresult(int test, int iteration)
{
    ...
    if ((test == TEST_WRITE100MB) || (test == TEST_READ100MB)) {
        if (t > 0) {  /* Prevent division by zero */
            ...
            fprintf(stderr, " for %lu MB (time too small to measure)", rwsize / (1024 * 1024));
        }
    }
...
static void displayresults(void)
{
    ...
    for (test = 0; test < NUMTESTS; test++) {
        ...
        if (test == TEST_WRITE100MB) {
            ...
            fprintf(stderr, "\n\t  Throughput: %llu MB/s (Write, %lu MB file)", thrput, rwsize / (1024 * 1024));
        }

        if (test == TEST_READ100MB) {
            thrput = (rwsize / 1000) / avg;
            fprintf(stderr, "\n\t  Throughput: %llu MB/s (Read, %lu MB file)", thrput, rwsize / (1024 * 1024));
        }
...
void run_test(const int dir)
{
    ...
    if (teststorun[TEST_CACHE_HITS]) {
        ...
        if (cache_dirs <= 0 || cache_dirs > 50 || cache_files_per_dir <= 0 || cache_files_per_dir > 50) {
        ...
        char (*cache_test_files)[32] = calloc(cache_dirs * cache_files_per_dir, sizeof(char[32]));
        ...
        if (!cache_test_files || !cache_test_dirs_arr) {
            ...
            if (cache_test_files) free(cache_test_files);
            if (cache_test_dirs_arr) free(cache_test_dirs_arr);
            fatal_failed();
        }
        ...
        for (i = 0; i < cache_dirs; i++) {
            for (j = 0; j < cache_files_per_dir; j++) {
                snprintf(cache_test_files[i * cache_files_per_dir + j], sizeof(cache_test_files[0]), "cache_file_%d_%d", i, j);
                if (FPCreateFile(Conn, vol, 0, cache_test_dirs_arr[i], cache_test_files[i * cache_files_per_dir + j])) {
                    free(cache_test_files);
                    free(cache_test_dirs_arr);
                    fatal_failed();
                }
        ...
        for (k = 0; k < cache_lookup_iterations; k++) {
            for (i = 0; i < cache_dirs; i++) {
                ...
                for (j = 0; j < cache_files_per_dir; j++) {
                    if (FPGetFileDirParams(Conn, vol, cache_test_dirs_arr[i], cache_test_files[i * cache_files_per_dir + j],
        ...
        for (i = 0; i < cache_dirs; i++) {
            for (j = 0; j < cache_files_per_dir; j++) {
                FPDelete(Conn, vol, cache_test_dirs_arr[i], cache_test_files[i * cache_files_per_dir + j]);
            }
    ...
    if (teststorun[TEST_MIXED_CACHE_OPS]) {
        ...
        for (i = 0; i < mixed_cache_files; i++) {
            ...
            if (i % 10 == 9) {  /* Every 10th iteration */
                if (FPEnumerate(Conn, vol, dir, "", (1 << FILPBIT_LNAME) | (1 << FILPBIT_FNUM), 0)) {
                    /* Enumeration failure is not fatal for this test, just log and continue */
                    fprintf(stderr, "Warning: Enumeration failed during mixed cache operations test (iteration %d)\n", i);
                }
            }
    ...
    if (teststorun[TEST_CACHE_VALIDATION]) {
        ...
        if (validation_files <= 0 || validation_files > 1000 ||
            ...
            fprintf(stderr, "Invalid validation test configuration: files=%d, iterations=%d\n",
