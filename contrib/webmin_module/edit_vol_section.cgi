...
my @tabs = (
            ['common', $text{'edit_vol_section_tab_common'}],
...
if ($subject eq 'volume') {
    ...
    my $select =
"<select name='p_vol preset' onchange='document.configform.action=\"edit_vol_section.cgi\"; document.configform.submit();'>\n"
      . "<option value='' "
...
if ($subject eq 'homes') {
    ...
    print &ui_table_row(
                        ...
                        &ui_textbox(
                                    'p_home name',
                                    (get_parameter_of_section($afpconfRef, $sectionRef, 'home name', \%in))[0], 40
                        )
    );
    print &ui_table_row(
                        ...
                        &ui_textbox(
                                    'p_basedir regex',
                                    (get_parameter_of_section($afpconfRef, $sectionRef, 'basedir regex', \%in))[0], 40,
                                    undef, undef, "required"
                        )
                        . " "
    ...
    print &ui_table_row(
                        ...
                        &ui_textbox(
                                    'p_path', (get_parameter_of_section($afpconfRef, $sectionRef, 'path', \%in))[0], 40
                        )
    );
    ...
    print &ui_table_row(
                        ...
                        &ui_textbox(
                                    'name', exists $in{name} ? $in{name} : ($sectionRef ? $$sectionRef{name} : ''), 40,
                                    undef,  undef, "required"
                        )
    );
} else {
    ...
    print &ui_table_row(
                        ...
                        &ui_textbox(
                                    'p_volume name', $volume_name ? $volume_name : $$sectionRef{'name'}, 40, undef,
                                    undef,           "required"
                        )
    );
}
...
if ($subject eq 'volume') {
    ...
    print &ui_table_row(
                        $text{'edit_vol_section_path'},
                        &ui_filebox(
                                    'p_path',
                                    exists $in{p_path} ? $in{p_path}
                                    : (
                                       exists $$sectionRef{parameters}{'path'} ? $$sectionRef{parameters}{'path'}{value}
                                       : ''
                                    ),
                                    40, undef, undef,
                                    "required",
...
if ($subject ne 'homes') {
    ...
    print &ui_table_row(
                        $text{'edit_vol_section_vol_size_limit'},
                        "<input name='p_vol size limit' type='number' value='"
                        . $values[0] . "'>" . " "
    ...
    print &ui_table_row(
                        $text{'edit_vol_section_password'},
...
print &ui_table_row(
                    ...
                    . "<b>$text{'edit_vol_section_file_perm'}</b> "
                    . &ui_textbox('p_file perm', $file_perm_values[0]) . "<i>"
                    . (
                        $file_perm_values[2]
                        ? html_escape($file_perm_values[1]) . " (" . html_escape($file_perm_values[2]) . ")"
                        : ''
                    )
                    . "</i><br>"
                    . "<b>$text{'edit_vol_section_directory_perm'}</b> "
                    . &ui_textbox('p_directory perm', $dir_perm_values[0]) . "<i>"
                    . (
                        $dir_perm_values[2]
                        ? html_escape($dir_perm_values[1]) . " (" . html_escape($dir_perm_values[2]) . ")"
                        : ''
                    )
                    . "</i><br>"
                    . "<b>$text{'edit_vol_section_umask'}</b> "
                    . &ui_textbox('p_umask', $umask_values[0]) . "<i>"
                    . (
                        $umask_values[2] ? html_escape($umask_values[1]) . " (" . html_escape($umask_values[2]) . ")"
                        : ''
                    )
                    . "</i>"
);
...
print &ui_table_row(
                    $text{'edit_vol_section_valid_users'},
                    &build_user_group_selection($afpconfRef, $sectionRef, \%in, 'valid users')
);
print &ui_table_row(
                    $text{'edit_vol_section_invalid_users'},
                    &build_user_group_selection($afpconfRef, $sectionRef, \%in, 'invalid users')
);
print &ui_table_row(
                    $text{'edit_vol_section_rolist'},
                    &build_user_group_selection($afpconfRef, $sectionRef, \%in, 'rolist')
);
print &ui_table_row(
                    $text{'edit_vol_section_rwlist'},
                    &build_user_group_selection($afpconfRef, $sectionRef, \%in, 'rwlist')
);

@values = get_parameter_of_section($afpconfRef, $sectionRef, 'hosts allow', \%in);
print &ui_table_row(
                    $text{'edit_vol_section_hosts_allow'},
...
print &ui_table_row(
                    $text{'edit_vol_section_hosts_deny'},
...
if ($subject ne 'homes') {
    ...
    print &ui_table_row(
                        $text{'edit_vol_section_veto_files'},
