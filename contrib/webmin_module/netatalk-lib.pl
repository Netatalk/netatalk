...
our %netatalkParameterDefaults = (
                                  ...
                                  'spotlight expr'              => 'yes',
                                  'srp verifier path'           => '/etc/netatalk/afppasswd.srp',
                                  'stat vol'                    => 'yes',
                                  'strict locking'              => 'no',
                                  'time machine'                => 'no',
...
our %netatalkParameterSelectOptions = (
                                       ...
                                       'multi protocol'             => [@yesNoSelectOptions],
...
sub write_afpconf {
    ...
    rename($config{'afp_conf'}, "$config{'afp_conf'}.orig")
      or die "rename failed: $config{'afp_conf'} -> $config{'afp_conf'}.orig $!\n";
    rename("$config{'afp_conf'}.tmp", $config{'afp_conf'})
      or die "rename failed: $config{'afp_conf'}.tmp -> $config{'afp_conf'}\n";
...
sub get_parameter_of_section {
    ...
    my $value =
      (defined $sectionRef && defined $$sectionRef{parameters}{$name}) ? $$sectionRef{parameters}{$name}{value} : '';
    ...
    if ($$sectionRef{name} !~ /Global/) {
        ...
        if ($inRef && exists $$inRef{"reload"}) {
            ...
            if (    $$sectionRef{parameters}{'path'}
                 && $$sectionRef{parameters}{'vol preset'}
                 && $$sectionRef{parameters}{'vol preset'}{value}) {
        ...
        unless (@searchList) {
            if (exists $$afpconfRef{sectionsByName}{'Global'}{parameters}{'vol preset'}
                 && $$afpconfRef{sectionsByName}{'Global'}{parameters}{'vol preset'}{value}) {
        ...
        while (@searchList) {
            ...
            if (exists $$afpconfRef{sectionsByName}{$sectionName}{parameters}{$name}
                 && $$afpconfRef{sectionsByName}{$sectionName}{parameters}{$name}{value}) {
    ...
    if ($index =~ /\d+/) {
        ...
        if ($sectionToModifyRef = ${$$afpconfRef{sectionsByIndex}}[$index]) {
            ...
            if ($existingName ne $name) {
                ...
                push @modlist,
                  [$$sectionToModifyRef{firstline}, $$sectionToModifyRef{linecountOfSectionHeader}, "[$name]"];
            }
        ...
        if ($key =~ /^p_(.*)/) {
            ...
            if ($$parametersOfSectionRef{$param_name}) {
                ...
                if ($value) {
                    push @modlist,
                      [
                       $$parametersOfSectionRef{$param_name}{firstline},
                       $$parametersOfSectionRef{$param_name}{linecount},
                       $$parametersOfSectionRef{$param_name}{keyAsInFile} . $value
                      ];
                } else {
                    push @modlist,
                      [
...
sub build_select {
    ...
    if (defined $textIfNoDefault) {
        $select .=
          ...
          . ($values[2]       ? html_escape($values[1]) . " (" . html_escape($values[2]) . ")" : $textIfNoDefault)
          . "</option>\n";
    }
