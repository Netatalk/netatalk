...
static int add_record(struct migrate_record **records, size_t *count,
                      ...
                      size_t source_length, size_t line_number)
{
    ...
    if (!srp_valid_username(record.username) ||
            !srp_valid_fields(colon + 1)) {
        goto malformed;
    }

    if (lookup_uid(record.username, &record.uid) < 0) {
        fprintf(stderr,
                        ...
                        size_t *count)
{
    ...
    while ((length = getline(&line, &size, stream)) >= 0) {
        ...
        if (source_length > SRP_USERNAME_MAX_LEN + SRP_FORMAT_LEN) {
            fprintf(stderr, "afppasswd: malformed legacy SRP record %zu.\n",
                    line_number);
            goto done;
        }

        if (source_length > 0 && line[source_length - 1] != '\n') {
            char *grown = realloc(line, source_length + 2);
            ...
            line = grown;
            size = source_length + 2;
            line[source_length] = '\n';
            length = (ssize_t)source_length + 1;
            line[length] = '\0';
        }

        if (add_record(records, count, &capacity, line, (size_t)length,
                       source_length, line_number) < 0) {
            goto done;
        }
    }
                            ...
                            size_t count)
{
    ...
    while ((length = read(source_fd, buffer, sizeof(buffer))) > 0) {
        for (ssize_t i = 0; i < length; i++) {
            ...
            if (offset == records[record].source_length) {
                record++;
                offset = 0;
            }
        }
                            ...
                            uid_t administrator_uid)
{
    ...
    for (size_t i = 0; i < count; i++) {
        struct stat st;
        int fd;
        /* Validated disabled records retain administrator ownership. */
        const char *fields = strchr(records[i].line, ':') + 1;
        uid_t owner = *fields == '*' ? administrator_uid : records[i].uid;
        ...
        if (fchmod(fd, 0600) < 0 ||
                write_all(fd, records[i].line, records[i].length) < 0 ||
        ...
        if (!S_ISREG(st.st_mode) || st.st_uid != owner ||
                !srp_verifier_mode_is_safe(st.st_mode) || st.st_nlink != 1) {
            fprintf(stderr,
                                    ...
                                    char temporary_name[MAXPATHLEN + 1])
{
    ...
    int length = snprintf(temporary_name, MAXPATHLEN + 1,
                          ".%s.migrate.%ju", basename, (uintmax_t)getpid());
    ...
    fd = openat(parent_fd, temporary_name,
                O_RDONLY | O_DIRECTORY | O_CLOEXEC | O_NOFOLLOW);
...
static enum restore_result restore_source(int parent_fd, const char *basename,
                                          const char *backup_name)
{
    ...
    if (unlinkat(parent_fd, backup_name, 0) < 0) {
        fprintf(stderr,
                ...
                backup_name);
        return RESTORE_CLEANUP_INCOMPLETE;
    }

    if (fsync(parent_fd) < 0) {
        fprintf(stderr,
                "afppasswd: rollback restored the source but could not synchronize removal of %s.\n",
                backup_name);
        return RESTORE_CLEANUP_INCOMPLETE;
    }
...
int afppasswd_migrate_srp(const char *path, uid_t administrator_uid)
{
    ...
    if (read_records(source_fd, &records, &count) < 0) {
        goto done;
    }
    ...
    if (temporary_fd < 0) {
        /* Creation failed or the helper already cleaned up its directory. */
        temporary_name[0] = '\0';
        goto done;
    }

    if (create_verifiers(temporary_fd, path, records, count,
                         administrator_uid) < 0 ||
    ...
    if (fsync(parent_fd) < 0 ||
        ...
        enum restore_result rollback_result;
        fprintf(stderr,
        ...
        if (rollback_result != RESTORE_FAILED) {
            source_unlinked = 0;
        }

        if (rollback_result == RESTORE_COMPLETE) {
            backup_name[0] = '\0';
        }
